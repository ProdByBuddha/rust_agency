...
const VAD_THRESHOLD: f32 = 0.015; // Adjusted for better environment robustness
const SILENCE_DURATION_MS: u64 = 800; // Faster trigger for better UX
...
#[tokio::main] async fn main() -> Result<()> {
    ...
    let subscriber = FmtSubscriber::builder()
        .with_env_filter("listener_server=debug,rust_agency=info")
        .finish();
    tracing::subscriber::set_global_default(subscriber).ok();

    info!("👂 Listener Server (Whisper SOTA) Starting...");
    ...
    // Real-time audio callback must be non-blocking
...
async fn process_speech(pcm: Vec<f32>, in_sample_rate: usize, state: Arc<AppState>) -> Result<()> {
    info!("⌛ Transcribing segment ({} samples)...", pcm.len());
    ...
    // 2. Transcribe using spawn_blocking for CPU-bound inference
    ...
    if text.is_empty() || text.len() < 2 {
        debug!("Ignoring empty/tiny transcription");
        return Ok(())
    }
    ...
    // 3. Send to Nexus
    let nexus_resp = state.client.post(NEXUS_URL)
        .json(&json!({
            "messages": [{"role": "user", "content": text}],
            "stream": true // Trigger streaming TTS on Nexus
        }))
        .send()
        .await;

    match nexus_resp {
        Ok(_) => info!("✅ Sent to Nexus Gateway"),
        Err(e) => error!("❌ Nexus connection failed: {}", e),
    }
